...
int _smallerIndexHeap(DynArr *heap, int i, int j, comparator compare)
{
  ...
  if(i < heap->size && j < heap->size){ //if the integers are greater
...
TYPE getMinHeap(DynArr *heap)
{
  ...
  if(heap != NULL && heap->size != 0){
	  //if(heap->size != 0){
		  return(heap->data[0]); //returns value of first node
	  }
}
...
void addHeap(DynArr *heap, TYPE val, comparator  compare)
{
  ...
  if(heap != NULL){
	  if(heap->size == heap->capacity){
		  _dynArrSetCapacity(heap, 2 * heap->capacity); //doubles the capacity
	  }
	  ...
	  while(curr != 0){ //organizes the heap
		  parent = (curr-1)/2;
		  if(_smallerIndexHeap(heap,curr,parent,compare) == curr){
			  swapDynArr(heap, parent, curr);
			  curr = parent;
		  }
		  else{
			  return;
...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{
  ...
  if(right < max){
	  curr = _smallerIndexHeap(heap,left,right,compare);
	  
	  if(_smallerIndexHeap(heap,curr,pos,compare) == curr){
		  swapDynArr(heap,curr,pos);
		  _adjustHeap(heap,max,curr,compare);
	  }
  }
  else if(left < max){
	  if(_smallerIndexHeap(heap, left, pos, compare) == left){
...
void removeMinHeap(DynArr *heap, comparator compare)
{
  ...
  if(heap != NULL && heap->size != 0){
	  heap->data[0] = heap->data[heap->size-1]; //fills empty
	  heap->size--;
...
{
  /* FIXME: Write This */
  int curr; //= (heap->size)/2 - 1;
  for (int curr = (heap->size)/2 - 1; curr >= 0; curr--){
		_adjustHeap(heap, heap->size, curr, compare);
	}
}
/*
...
void sortHeap(DynArr *heap, comparator compare)
{
  ...
  for(curr = heap->size - 1; curr >= 0; curr--){
	  swapDynArr(heap,0,curr);
	  _adjustHeap(heap,curr,0,compare);
  }
