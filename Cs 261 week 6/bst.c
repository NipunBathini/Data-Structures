...
struct Node *_addNode(struct Node *cur, TYPE val)
{
	...
	if(cur == NULL){
		...
		cur->right = NULL;
		return(cur);
		printf("Added to the root\n");
	}
	else{
		if(compare(cur->val,val)==1){ //a lower value will go to the left
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	...
	while(cur != NULL){
		...
		else if(compare(cur->val, temp->val)  == -1){
			if(cur->right != NULL){
				cur = cur->right;
			}
			else{
		...
		else if(compare(cur->val, temp->val) == 0){
			return 1;
		}
	}
...
{
    /*write this*/
	while(1){ //have to loop through left most
		if(cur->left == NULL){
			return(cur->val);
		}
		else{
