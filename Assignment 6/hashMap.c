...
{
	/*write this*/
	struct hashLink *linkIterator;
	struct hashLink *temp;
	
	int i;
	for(i=0; i< tableSize; i++){
		linkIterator = table[i];
		
		while(linkIterator != NULL){
			temp = linkIterator->next;
			free(linkIterator);
			linkIterator = temp;
		}
	}
...
void _setTableSize(struct hashMap * ht, int newTableSize)
{
	...
	for(i = 0; i< ht->tableSize; i++){
		if(ht->table[i] != NULL){
			...
			while(itr != NULL){
				insertMap(new, itr->key, itr->value); //set
				itr = itr->next;
			}
		}
	}
...
void insertMap (struct hashMap * ht, KeyType k, ValueType v)
{
	...
	if(HASHING_FUNCTION == 1){ // if theyre changed
	...
	if(ht->table[hashIdx] != NULL){
		struct hashLink* itr = ht->table[hashIdx];
...
ValueType atMap (struct hashMap * ht, KeyType k)
{
	...
	if(ht->table[hashIdx] != NULL){
		...
		while(1){
			...
			else if(strcmp(itr->key, k) == 0){ //if theres a match
				return itr->value;
			}
			else{
				itr = itr->next;
...
void removeKey (struct hashMap * ht, KeyType k)
{
	...
	if(ht->table[hashIdx] != NULL){
		...
		if(strcmp(itr->key, k) == 0){
			struct hashLink* remove = itr;
			ht->table[hashIdx] = remove->next;
			free(remove);
			ht->count--; //decrease
			return; 
		}
		...
		if(itr->next == NULL){
			...
			struct hashLink* remove = itr->next;
			itr->next = remove->next;
			free(remove);
			ht->count--;
		}
	}
...
{
	/*write this*/
	return (ht->count); //returns the number of hashLinks in the table
...
{
	/*write this*/
	return (ht->tableSize);//returns the number of buckets in the table
}
...
{
	/*write this*/
	return ((float)ht->count/ht->tableSize); //error without float
}
