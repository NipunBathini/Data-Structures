...
int main (int argc, const char * argv[]) {
	...
	clock_t timer;
	FILE *fileptr;
	char *word;//for characters
	int *value; //for integeres
	int test;
	...
	do{
		...
		if(word != NULL){// for first time around
			test = containsKey(hashTable, word);
			if(test == 0){
				value = malloc(sizeof(int));
				*value = 1; //pointer of thei integer value will be 10
				
				insertMap(hashTable, word, value);
			}
			else{
				value = atMap(hashTable, word);
				*(int *)value = *(int *)value + 1;// increase value
			}
		}
	}
	while(word != NULL);
